(function () {
    function compileAnimationJson(project) {
        var animations = {};
    ...
    function calculateVisibleBox(project) {
        ...
        width = Math.max(width, project.visible_box[0] || 0);
        ...
        y_min = Math.min(
            y_min,
            project.visible_box[2] - project.visible_box[1] / 2 || 0
        );
        y_max = Math.max(
            y_max,
            project.visible_box[2] + project.visible_box[1] / 2 || 0
            ...
            for (var key in base_cube.faces) {
                ...
                if (s.uv[key]) {
                    face.extend({
                        ...
                        uv: [s.uv[key].uv[0], s.uv[key].uv[1]],
                    ...
                    face.texture = null;
                    (face.uv = [0, 0, 0, 0]), (face.rotation = 0);
                }
            }
        }
            ...
            for (let key in b.locators) {
                ...
                if (b.locators[key] instanceof Array) {
                    ...
                    ignore_inherited_scale =
                        b.locators[key].ignore_inherited_scale;
                }
                coords[0] *= -1;
                ...
                if (
                    key.substr(0, 6) == "_null_" &&
                    b.locators[key] instanceof Array
                ) {
                    new NullObject({ from: coords, name: key.substr(6) })
                        .addTo(group)
                        .init();
                } else {
                    new Locator({
    ...
    function parseGeometry(data) {
        ...
        let geometry_name =
            (description.identifier &&
                description.identifier.replace(/^geometry\./, "")) ||
            "";
        ...
        if (
            typeof description.visible_bounds_width == "number" &&
            typeof description.visible_bounds_height == "number"
        ) {
            Project.visible_box[0] = Math.max(
                Project.visible_box[0],
                description.visible_bounds_width || 0
            );
            Project.visible_box[1] = Math.max(
                Project.visible_box[1],
                description.visible_bounds_height || 0
            );
            if (
                description.visible_bounds_offset &&
                typeof description.visible_bounds_offset[1] == "number"
            ) {
                Project.visible_box[2] =
                    description.visible_bounds_offset[1] || 0;
            }
        }
        ...
        Project.box_uv =
            Cube.all.filter((cube) => cube.box_uv).length > Cube.all.length / 2;
            ...
            for (var key in cube.faces) {
                ...
                if (face.texture !== null) {
                    template.uv[key] = new oneLiner({
                        ...
                        uv_size: [face.uv_size[0], face.uv_size[1]],
                    });
                    if (face.rotation) {
    ...
    function compileGroup(g, bones, config, project) {
        if (g.type !== "group" || g.export == false) return;

        if (
            !settings.export_empty_groups.value &&
            !g.children.find((child) => child.export)
        )
            return;
                ...
                if (obj instanceof Cube) {
                    ...
                    for (var face in obj.faces) {
                        if (obj.faces[face].texture !== null) {
                            texture = project.textures.findInArray(
                                "uuid",
                                obj.faces[face].texture
                            );
                            ...
                            let texture_name_clean = texture.name.replace(
                                "_e.png",
                                ".png"
                            );
                    ...
                    if (textures_cubes[texture.name] == null) {
                        var cube_name =
                            "uv_" +
                            bone.name +
                            Object.keys(textures_cubes).length;
                    ...
                    if (
                        (obj.rotatable && !obj.rotation.allEqual(0)) ||
                        obj.ignore_inherited_scale
                    ) {
                        locators[key] = {
                            offset,
                        };
    ...
    function getFormatVersion(project) {
        if (Format.display_mode) {
            let has_new_displays = false;

            if (project.display_settings && project.display_settings.slots) {
                for (let i in project.display_settings.slots) {
                    let key = project.display_settings.slots[i];
                    if (
        ...
        if (project.elements) {
            for (let cube of project.elements.filter(
                (el) => el instanceof Cube
            )) {
        ...
        if (project.groups) {
            if (project.groups.find((group) => group.bedrock_binding))
                return "1.16.0";
        }
        return "1.12.0";
    }
    ...
    var codec = {
        ...
        compile(options, config, project) {
            if (options === undefined) options = {};

            var entitymodel = {};
            var main_tag = {
                format_version: getFormatVersion(project),
            ...
            if (project.elements) {
                project.elements.forEach((obj) => {
                    if (
                        obj instanceof OutlinerElement &&
                        obj.parent === "root"
                    ) {
            ...
            if (loose_elements.length) {
                ...
                projectGroups.splice(0, 0, group);
            }

            projectGroups.forEach(function (g) {
                let bone = compileGroup(g, bones, config, project);
                if (bone !== undefined) {
                    bones.push(bone);
                }
            });
            ...
            if (bones.length) {
                let visible_box = calculateVisibleBox(project);
                entitymodel.description.visible_bounds_width =
                    visible_box[0] || 0;
                entitymodel.description.visible_bounds_height =
                    visible_box[1] || 0;
    ...
    BBPlugin.register("geyser_model_engine_packer_all", {
        title: "GeyserModelEnginePackerAll",
        author: "ElMando",
        icon: "bar_chart",
        description:
            "Exports all open Blockbench projects as nested GeyserModelEngine models.",
        tags: ["export", "model engine", "geyser"],
        version: "0.0.1",
        min_version: "4.8.0",
        variant: "both",
        onload() {
            var export_action = new Action({
                id: "export_geysermodelengine_all",
                name: "Export GeyserModelEngine Model (All open projects)",
                icon: "icon-format_bedrock",
                category: "file",
                click: async function () {
                    const main_zip_name = `geyser_model_pack_${Date.now()}`;
                    const final_outer_zip = new JSZip();

                    for (const proj of ModelProject.all) {
                        let project_folder_name = proj.name.replace(
                            /[^a-zA-Z0-9_\-]/g,
                            "_"
                        );

                        let inner_model_folder =
                            final_outer_zip.folder(project_folder_name);
                        ...
                        if (proj.textures) {
                            proj.textures.forEach((texture) => {
                                ...
                                model_config.per_texture_uv_size[name] = [
                                    texture.uv_width,
                                    texture.uv_height,
                                ];
                                inner_model_folder.file(
                                    name + ".png",
                                    texture.getBase64(),
                                    {
                                        base64: true,
                        ...
                        inner_model_folder.file(
                            proj.name + ".geo.json",
                            JSON.stringify(
                                codec.compile(null, model_config, proj)
                            )
                        );

                        inner_model_folder.file(
                            proj.name + ".animation.json",
                            JSON.stringify(compileAnimationJson(proj))
                        );
                    ...
                    const mainContent = await final_outer_zip.generateAsync({
                        type: "blob",
                    });
                    ...
                    Blockbench.export({
                        type: "Zip Archive",
                        extensions: ["zip"],
                        name: main_zip_name + ".zip",
                        savetype: "zip",
                        content: mainContent,
                    });

                    Blockbench.notification("Export complete!");
                },
            });

            MenuBar.addAction(export_action, "file.export");
            console.log("GeyserModelEnginePackerAll plugin loaded!");
        },
        onunload() {
